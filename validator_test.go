...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "few validate in one tag correct",
			args: args{v: struct {
				A int `validate:"max:5;min:3;in:3,4,5,6,7"`
			}{
				4,
			}},
			wantErr: false,
			checkErr: func(err error) bool {
				assert.Len(t, err.(ValidationErrors), 0)
				return true
			},
		},
		{
			name: "few validate in one tag wrong",
			args: args{v: struct {
				A int `validate:"max:5;min:3;in:3,4,5,6,7"`
			}{
				10,
		...
		{
			name: "correct slice with many validate",
			args: args{v: struct {
				A []int    `validate:"min:2;max:5;in:1,2,3"`
				B []string `validate:"min:3;max:10;in:sd11,dg222"`
			}{
				[]int{3, 2},
				[]string{"dg222", "sd11"},
			}},
